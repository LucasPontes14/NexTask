...
class TaskRepository:
    ...
    def adicionar(self, tarefa: Tarefa) -> int:
        ...
        cursor.execute(
            """
            INSERT INTO tarefa (id_lista, titulo, descricao, prioridade, status, data_venc, hora_venc)
    ...
    def listar_todas(self) -> list[Tarefa]:
        ...
        cursor.execute(
            "SELECT * FROM tarefa ORDER BY id_tarefa"
        )
    ...
    def atualizar(self, tarefa: Tarefa) -> bool:
        ...
        cursor.execute(
            ...
            SET id_lista = %s, titulo = %s, descricao = %s,
                prioridade = %s, status = %s, data_venc = %s, hora_venc = %s
            WHERE id_tarefa = %s
            """,
            (
                tarefa.id_lista,
